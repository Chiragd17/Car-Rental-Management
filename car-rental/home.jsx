...
const QUICK_FILTERS = [
  { label: '⚙️ Automatic', params: { transmission: 'Automatic' } },
  { label: '💺 5+ Seats', params: { seats: '5+' } },
  { label: '⚡ Electric', params: { fuel_type: 'Electric' } },
  { label: '🏆 Premium Fleet', params: { tier: 'Premium' } },
]
...
export default function Home() {
  ...
  const applyFilter = (params) => {
    navigate(`/results?${new URLSearchParams(params)}`)
  }

  return (
    <div className="min-h-screen bg-cream">
      {/* ── Hero ──────────────────────────────────────────────────────────── */}
      <section className="hero-section relative min-h-screen flex flex-col items-center justify-center">
        {/* Background */}
        <div
          className="absolute inset-0 bg-cover bg-center"
          style={{
            backgroundImage: `url('https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?w=1800&q=85')`,
          }}
        />
        {/* Gradient overlay */}
        ...
        <div className="relative z-10 text-center px-6 mb-10 mt-16">
          <span className="inline-block text-orange font-semibold text-sm tracking-[0.2em] uppercase mb-4 opacity-90">
            Premium Experience
          </span>
          <h1 className="font-display text-5xl md:text-7xl font-bold text-white leading-tight max-w-4xl mx-auto">
            Drive in
            <span className="text-orange italic"> Style,</span>
            <br />
            Wherever You Go
          </h1>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {QUICK_FILTERS.map(({ label, params }) => (
              <button
                key={label}
                onClick={() => applyFilter(params)}
                className="bg-white border border-gray-100 rounded-2xl p-5 text-left
                  hover:border-orange/30 hover:shadow-md transition-all group"
              >
                <div className="text-2xl mb-3">{label.split(' ')[0]}</div>
                <div className="font-semibold text-forest text-sm group-hover:text-orange transition-colors">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              { icon: '⚡', title: 'Instant Booking', desc: 'Book your vehicle in under 2 minutes with our streamlined checkout flow.' },
              { icon: '📍', title: '50+ Pickup Locations', desc: 'Conveniently placed across major airports, railway stations, and city centres.' },
            ].map(({ icon, title, desc }) => (
