...
export default function Navbar() {
  const { user, isAdmin, logout, setShowModal, requireAuth } = useAuth()
      ...
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between relative">
        ...
        <Link to="/" className="flex items-center gap-2">
          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold
            ${isHome && !scrolled ? 'bg-white text-forest' : 'bg-forest text-white'}`}>
            D
          </div>
        ...
        {/* Nav Links */}
        <div className={`hidden md:flex items-center gap-8 text-sm font-medium absolute left-1/2 -translate-x-1/2 ${textCls}`}>          
          <Link to="/results" className="opacity-80 hover:opacity-100 transition-opacity">
            Browse Cars
          </Link>
          <button
            onClick={() => requireAuth(() => navigate('/bookings'))}
            className="opacity-80 hover:opacity-100 transition-opacity">
            My Bookings
          </button>
          {user && (
            <Link to="/profile" className="opacity-80 hover:opacity-100 transition-opacity">
              Profile
            </Link>
          )}
          {isAdmin && (
            <Link to="/admin"
              className="font-semibold px-3 py-1 rounded-full text-xs border border-orange/40 text-orange bg-orange/10 hover:bg-orange/20 transition-all">
              ⚙️ Admin
            </Link>
          )}
        </div>
